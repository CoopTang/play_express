...
const index = async (req, res) => {
  ...
  let playlist = await database('playlists').where({id: playlistId}).first()

  if (playlist === undefined) {
    return res.status(404).json({ message: "Playlist with that ID does not exist!"})
  } else {
    Playlist.formattedPlaylistInfo(playlistId)
      .then(payload => {
